...
declare -a SUPPLIES=('VDD_DSPEVE  ' 'VDD_MPU     ' '1V35_DDR    ' 'VDDA_1V8_PLL' 'VDD_GPU     ' 'VUSB3V3     ' 'VDDS18V     ' 'VDDSHV      ' 'CORE_VDD    ' 'VDD_IVA     ' 'VDD_DDR     ' 'VDDA_1V8_PHY');
#declare -a SUPPLIES=('VDD_DSPEVE' 'VDD_MPU' '1V35_DDR' 'VDDA_1V8_PLL' 'VDD_GPU' 'VUSB3V3' 'VDDS18V' 'VDDSHV' 'CORE_VDD' 'VDD_IVA' 'VDD_DDR' 'VDDA_1V8_PHY');
...
for ((i=0 ; i<$NUM_SUPPLIES ; i++)); do
	printf "%s\t%.3f\t\t%f\t%f\t%f\t%f\n" "${SUPPLIES[i]}" "${RES[i]}" "${SUPPLIES_SHUNTV[i]}" "${SUPPLIES_BUSV[i]}" "${SUPPLIES_CURRENT[i]}" "${SUPPLIES_POWER[i]}"
done
